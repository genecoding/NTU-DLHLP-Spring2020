...
class BertForTokenClassification(BertPreTrainedModel):
    ...
    Examples::
        ...
        # since it will affect training, I don't think it should be put here, anyway...
        
        # transition = torch.tensor(
        #     [[-float('Inf'), 0, 0, -float('Inf')],
        #      [-float('Inf'), 0, 0, -float('Inf')],
        #      [0, -float('Inf'), -float('Inf'), 0],
        #      [0, -float('Inf'), -float('Inf'), 0]], device=logits.device)
        
        # # logits.shape: (batch_size, 128 (# of words), # of tags)
        
        # for i in range(logits.shape[1]):
        #     if i == 0:  # token [CLS]
        #         continue
        #     elif i == 1:  # tag of the first word
        #         previous_tag = logits[:, i].argmax(dim=1)
        #     else:
        #         logits[:, i] += transition[previous_tag]
        #         previous_tag = logits[:, i].argmax(dim=1)
