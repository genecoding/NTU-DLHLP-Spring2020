...
config = BertConfig.from_pretrained('./output/config.json')
model = BertForTokenClassification.from_pretrained('./output/pytorch_model.bin', config=config).to(device)
model.eval()
...
def seg(sentence):
    ...
    ##### post-processing of sequence labeling #####
    # this shouldn't be put in modeling_bert.py, since it will affect training.
    # it's better to be put here.
    
    transition = torch.tensor(
        [[-float('Inf'), 0, 0, -float('Inf')],
         [-float('Inf'), 0, 0, -float('Inf')],
         [0, -float('Inf'), -float('Inf'), 0],
         [0, -float('Inf'), -float('Inf'), 0]], device=device)
    
    for i in range(paraphrase_classification_logits.shape[1]):
      if i == 0:  # token [CLS]
        continue
      elif i == 1:  # tag of the first word
        previous_tag = paraphrase_classification_logits[0, i].argmax()
      else:
        paraphrase_classification_logits[0, i] += transition[previous_tag]
        previous_tag = paraphrase_classification_logits[0, i].argmax()
