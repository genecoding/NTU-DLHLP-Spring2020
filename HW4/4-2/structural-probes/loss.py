...
class RankDepthLoss(nn.Module):
  ...
  def forward(self, predictions, label_batch, length_batch):
    ...
    if total_sents > 0:
      loss_per_sent = 1 - torch.sign(labels_masked.unsqueeze(dim=2)-labels_masked.unsqueeze(dim=1))\
                      * (predictions_masked.unsqueeze(dim=2)-predictions_masked.unsqueeze(dim=1))
      # mask places of no word and sum up, shape: (batch_size)
      loss_per_sent = (loss_per_sent * labels_1s.unsqueeze(dim=1)).triu(diagonal=1).sum(dim=(1, 2))
      pairs_per_sent = length_batch * (length_batch-1) / 2
      normalized_loss_per_sent = loss_per_sent / pairs_per_sent
      # mask the case that sentence length is 0, which would result in nan(0/0) or inf(/0)
      normalized_loss_per_sent = normalized_loss_per_sent[torch.isfinite(normalized_loss_per_sent)]
      batch_loss = torch.sum(normalized_loss_per_sent) / total_sents
    else:
      batch_loss = torch.tensor(0.0, device=self.args['device'])
    return batch_loss, total_sents
