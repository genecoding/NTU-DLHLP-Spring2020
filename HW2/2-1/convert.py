...
def convert_sp_inter(sp, c1, c2, solver, gen=True):
	c1_var = Variable(torch.from_numpy(np.array([c1]))).cuda()
	c2_var = Variable(torch.from_numpy(np.array([c2]))).cuda()
	sp_tensor = torch.from_numpy(np.expand_dims(sp, axis=0))
	sp_tensor = sp_tensor.type(torch.FloatTensor)
	converted_sp = solver.test_step(sp_tensor, c1_var, c2_var, gen=gen)
	converted_sp = converted_sp.squeeze(axis=0).transpose((1, 0))
	return converted_sp
...
def convert_all_sp(h5_path, src_speaker, tar_speaker, solver, dir_path,
	...
	with h5py.File(h5_path, 'r') as f_h5:
		...
		for utt_id in f_h5[f'{dset}/{src_speaker}']:
			...
			# add for interpolation of p1 & p2	
			converted_sp = convert_sp_inter(sp, speaker2id[tar_speaker], speaker2id[src_speaker], solver, gen=gen)
			wav_data = sp2wav(converted_sp)
			wav_path = os.path.join(dir_path, f'{src_speaker}_{tar_speaker}_{utt_id}_inter.wav')
			wavfile.write(wav_path, 16000, wav_data)
		...
		for speaker_B in speakers:
			if speaker_A != speaker_B:
				...
				convert_all_sp(h5_path, speaker_A, speaker_B,
								solver, dir_path, gen=False, max_n=max_n)  # run without gen
