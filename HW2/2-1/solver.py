...
class Solver(object):
    ...
    def build_model(self):
        ...
        # add for interpolation of p1 & p2
        self.Decoder_with_Interpolation = cc(Decoder_with_Interpolation(ns=ns, c_a=hps.n_speakers, emb_size=emb_size))
        self.Generator_with_Interpolation = cc(Decoder_with_Interpolation(ns=ns, c_a=hps.n_speakers, emb_size=emb_size))
    ...
    def load_model(self, model_path, enc_only=True):
        ...
        with open(model_path, 'rb') as f_in:
            ...
            # add for interpolation of p1 & p2
            self.Decoder_with_Interpolation.load_state_dict(all_model['decoder'])
            self.Generator_with_Interpolation.load_state_dict(all_model['generator'])
    ...
    def test_step(self, x, c1, c2=None, gen=False):
        self.set_eval()
        x = to_var(x).permute(0, 2, 1)
        enc = self.Encoder(x)
        # modify for interpolation of p1 & p2
        x_tilde = self.Decoder(enc, c1) if c2 == None else self.Decoder_with_Interpolation(enc, c1, c2)
        if gen:
            x_tilde += self.Generator(enc, c1) if c2 == None else self.Generator_with_Interpolation(enc, c1, c2)
        ...
        if mode == 'pretrain_G':
            ...
            for iteration in range(hps.patch_iters):
                ...
                if iteration % 100 == 0:
                    for tag, value in info.items():
                        self.logger.scalar_summary(tag, value, iteration + 1)
                if iteration % 2000 == 0 or iteration + 1 == hps.patch_iters:
            ...
            for iteration in range(hps.iters):
                ...
                if iteration % 100 == 0:
                    for tag, value in info.items():
                        self.logger.scalar_summary(tag, value, iteration + 1)
                if iteration % 2000 == 0 or iteration + 1 == hps.iters:
