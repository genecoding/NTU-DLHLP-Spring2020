...
class Inferencer(object):
    ...
    def inference_from_path(self):
        # modified for multiple source & target files
        ...
        with open(self.args.target, 'r') as f:
            tar = f.readlines()

        for n, wav in enumerate(zip(src, tar)):
            src_mel, _ = get_spectrograms(wav[0][:-1])  # [:-1] for removing '\n'
            tar_mel, _ = get_spectrograms(wav[1][:-1])
            ...
            self.write_wav_to_file(conv_wav, f'{self.args.output}/output{n}.wav')
